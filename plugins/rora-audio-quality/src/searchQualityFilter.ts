...
const findRecentSearchHeadings = (): HTMLElement[] =>
	[...document.querySelectorAll<HTMLElement>("h1, h2, h3, h4, strong, div, span")]
		.filter((element) => element.children.length === 0)
		.filter((element) => element.textContent?.trim().toLowerCase() === "recent searches");
...
export class SearchQualityFilter {
	...
	constructor(
		private readonly loadQuality: (trackId: string) => Promise<TrackAudioQuality>,
	) {}
	...
	stop(): void {
		...
		this.observer = null;
		document.querySelectorAll<HTMLElement>(".rora-search-quality-control").forEach((control) => {
			control.parentElement?.classList.remove("rora-search-quality-heading");
			control.remove();
		});
		document.querySelectorAll<HTMLElement>("[data-rora-quality-filtered]").forEach((element) => {
	...
	private mount(): void {
		for (const heading of findRecentSearchHeadings()) {
			...
			if (!host || !root || host.querySelector(".rora-search-quality-control")) continue;
	...
	private async apply(root: HTMLElement, selected: FilterValue): Promise<void> {
		...
		const links = [...root.querySelectorAll<HTMLAnchorElement>('a[href*="/track/"]')];
		const targets = new Map<HTMLElement, string>();
		for (const link of links) {
			const trackId = link.href.match(/\/track\/(\d+)/)?.[1];
